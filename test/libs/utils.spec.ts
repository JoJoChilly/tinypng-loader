...
describe('utils', () => {
    describe('string2Uint8array', () => {
        it('parameter(s) <hello> should derive return <null>', () => {
            const input = `hello`;
            expect(JSON.stringify(string2Uint8array(input))).to.equals(
        ...
        it('parameter(s) <0> should derive return <null>', () => {
            const input = 0;
            expect(() => string2Uint8array(input as any)).to.throw(
                `utils: unknown parameter type. expect parameter type to be string`,
            );
        });
        ...
        it('parameter(s) <[]> should derive to throw', () => {
            const input = [];
            expect(() => string2Uint8array(input as any)).to.be.throw(
                `utils: unknown parameter type. expect parameter type to be string`,
            );
        });
